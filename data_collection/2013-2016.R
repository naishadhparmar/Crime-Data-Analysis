getdata.2013 <- function(crimes_links, year) {
  library(httr)
  ...
  for(i in nrow(crimes):1) {
    if(!is.na(crimes[i, 1])) rows_to_remove <- append(rows_to_remove, i)
    else break
  }
    ...
    if(new_state == '') new_state <- gsub("[0-9]*", "", crimes[i, 1])
      ...
      if(!is.na(crimes[i, 3]) & crimes[i, 3] != 'None') {
        row <- data.frame(Year=c(year), State=c(new_state), Area=c(area), Population=c(crimes[i, 3]),
                          Violent=c(crimes[where, 4]), Property=c(crimes[where, 10]), Murder=c(crimes[where, 5]),
                          Rape=c(crimes[where, 6]), Robbery=c(crimes[where, 8]), Assault=c(crimes[where, 9]),
