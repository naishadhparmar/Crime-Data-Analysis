...
getdata.1999 <- function(crimes_links, year) {
  ...
  for(i in 1:nrow(crimes)) {
    if(sum(is.na(crimes[i, ])) >= ncol(crimes)-1) rows_to_remove <- append(rows_to_remove, i)
    else break
  }
  ...
  while(i <= nrow(crimes)) {
    if(new_state == '') {
      ...
      else if(grepl('Estimated total', crimes[i+2, 1], fixed=TRUE)) where <- i+2
      else if(grepl('Area actually reporting', crimes[i+1, 1], fixed=TRUE)) where <- i+1
      else where <- i
      if(!is.na(crimes[i, 2]) & crimes[i, 2] != 'None') {
        row <- data.frame(Year=c(year), State=c(new_state), Area=c(area), Population=c(crimes[i, 2]),
                          Violent=c(crimes[where, 5]), Property=c(crimes[where, 6]), Murder=c(crimes[where, 7]),
                          Rape=c(crimes[where, 8]), Robbery=c(crimes[where, 9]), Assault=c(crimes[where, 10]),
                          Burglary=c(crimes[where, 11]), Theft=c(crimes[where, 12]), Motor=c(crimes[where, 13]),
                          Arson=c(crimes[where, 14]))
  ...
  colnames(crimes_condensed) <- c('Year', 'State', 'Area', 'Population', 'Violent', 'Property',
                                  'Murder', 'Rape', 'Robbery', 'Assault', 'Burglary', 'Theft', 'Motor', 'Arson')
